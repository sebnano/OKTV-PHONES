package com.app.oktv.custom.widget
...
class CustomTextViewSemiBold  : AppCompatTextView {
    ...
    private fun selectTypeface(context: Context, textStyle: Int): Typeface {
        return when (textStyle) {
            Typeface.BOLD -> Typeface.createFromAsset(context.assets, "poppins_semiBold.ttf")
            Typeface.NORMAL -> Typeface.createFromAsset(context.assets, "poppins_semiBold.ttf")// regular
            else -> Typeface.createFromAsset(context.assets, "poppins_semiBold.ttf")// regular
        }
    }
